...
{
  .Object@ParameterNames = c('CommissionableSums','balance')
  .Object@SelectedCharacteristics = c('CommissionableSums','balance')
...
setMethod('setSelection','ContrastA', function(tObject, dataset, input)
{
  ...
  tObject@Selected$impervious <- -1 * tObject@Selected$CommissionableSums * tObject@Selected$balance
...
setMethod("Visualize_Essence",'ContrastA', function(.Object, filter, lens){
  ...
  Contrasts1 = renderPlot({ 
    twoord.plot(appel$sequence,appel$balance,
                appel$sequence,appel$CommissionableSums, type='l', lcol = 'red', 
